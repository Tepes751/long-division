...
public class Division {
	...
	List<PairNumber> divisionDetailsList;
	...
	private String createMiddlePart(){
		...
		for(int i = 1; i < divisionDetailsList.size() - 1; i++){
			PairNumber pairNumber = divisionDetailsList.get(i);
	...
	private void divideEvently(int dividend, int divisor){		
		...
		PairNumber tempPairNumber = new PairNumber(topTemp, bottomTemp);		
		divisionDetailsList.add(tempPairNumber);	
											
		if(!stringDididend.isEmpty()){			
			if(stringDididend.startsWith("0") && tempPairNumber.getSubtraction() == 0){
				while(stringDididend.startsWith("0")){					
					divisionResult = divisionResult + stringDididend.substring(0, 1);
					stringDididend = stringDididend.substring(1);
								
					divisionDetailsList.add(new PairNumber(0, 0));
				}				
			} 
			
			divisionResult += repeat(String.valueOf(divisor).length() 
					- String.valueOf(tempPairNumber.getSubtraction()).length() - 1, '0');
			
			if(!stringDididend.isEmpty()){
				divideEvently(Integer.parseInt(tempPairNumber.getSubtraction() + stringDididend), divisor);
			}
			
		} else {			
			int subtraction = tempPairNumber.getSubtraction();			
			PairNumber lastTempPairNumber = new PairNumber(subtraction, 0);			
	...
	private void divideResidue(int dividend, int divisor){
		...
		if (accuracy > 0){
			...
			if (findTopNumber(topTemp) >= 0 && accuracy < 10){						
				PairNumber tempPairNumber = new PairNumber(dividend, 0);		
				divisionDetailsList.add(tempPairNumber);
				...
				if (accuracy > 0){					
					...
					if(tempPairNumber.getSubtraction() == 0 || accuracy == 1){		
						PairNumber lastTempPairNumber = new PairNumber(tempPairNumber.getSubtraction(), 0);			
						divisionDetailsList.add(lastTempPairNumber);						
					} else {						
						divideResidue(tempPairNumber.getSubtraction(), divisor);
					}					
				}				
			}
